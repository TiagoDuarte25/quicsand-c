...
void * stream_write(void *arg) {
    ...
    while (1) {
        ...
        stream_io->ready = 0;
        log_trace("[strm][%p] stream_write: sending %d bytes on stream %p", (void *)stream_io->stream , len, stream_io->stream);
        pthread_mutex_lock(&ctx->mutex);
...
void* client_recv_cb(void *arg) {
    ...
    while (1) {
        ...
        if (quiche_conn_is_established(conn_io->conn)) {
            ...
            log_trace("stream %" PRIu64 " erro_code: %lu", s, error_code);
            }
            quiche_stream_iter_free(readable);
        }
        pthread_mutex_unlock(&conn_io->mutex);
    ...
    while (1) {
        ...
        if (len <= 0) {
            ...
            if (quiche_conn_stream_send(conn_io->conn, stream_io->stream_id, (const uint8_t *)close_msg, strlen(close_msg), true, &error_code) < 0) {
                log_error("failed to finish the stream: %lu", error_code);
...
void destroy_quic_context(context_t context) {
    ...
    timeout.tv_nsec = 0;
    while (g_hash_table_size(ctx->connections) > 0) {
